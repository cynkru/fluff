...
class UnknownEventWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (event.type == 'm.room.create') {
      return _buildRoomCreateEvent(context);
    }
    ...
    if (event.redacted) {
      ...
      if (reason != null && reason.isNotEmpty) {
        return _buildRedactedWidget(context, reason);
      }
  ...
  Widget _buildMemberEvent(BuildContext context) {
    final theme = Theme.of(context);
    final displayName = event.content['displayname'] as String? ?? 'Пользователь';
  ...
  Widget _buildRoomCreateEvent(BuildContext context) {
  ...
  Widget _buildRedactedWidget(BuildContext context, String reason) {
    final theme = Theme.of(context);
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
      padding: const EdgeInsets.all(8),
...
/// Проверяет, является ли событие системным
bool isSystemEvent(Event event) {
  const systemTypes = [
    'm.room.name',
    'm.room.topic',
    'm.room.avatar',
